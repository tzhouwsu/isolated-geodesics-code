...
!**********************************************************************
      ...
      integer :: i,j,k,nnodes,sumEdge,sumselect
      integer, dimension(500) :: hold
...
!      CALL execute_command_line("rm nlines.dat")
      ...
      nlines = 100000;    ! this number is enough for non-polarizable, 03.06, Tiecheng
...
!      command = trim(inputfile)//mand
      ...
      ! added the following print information, Tiecheng
      mand = "count."//trim(inputfile)
      open(unit=22,file=mand,action="write", iostat=er)
        if(er /= 0) then
          write(6, '(a)') 'cannot open outputfile for a snapshot'
          stop
        endif

      sumselect = 0;
      do i = 1,nnodes
        if( (i .ge. 148) .and. (i .le. 294) ) then 
          sumselect = sumselect + edge(i)
          write(22,'(I3 I8)') i, edge(i)
        endif
      enddo
